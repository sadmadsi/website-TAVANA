  ...
  return (
    <>
      <div className="justify-start basis-1/4 ">
        <Link
          href={"https://tavanaco.ir/files/download/rahnama-hamrah.pdf"}
        ...
        <Link
          href={"https://tavanase.etadbir.com/login.html"}
          className="appbar-item"
        >
        ...
        <Link
          href={"https://c.onlinetavana.ir/User/Login?ReturnUrl=/"}
          className="appbar-item"
        >
      ...
      <Link href={"https://sejam.tavanaco.ir/send-otp"} className="grow">
        ...
        <p className="mt-3 text-center -translate-y-7 font-extrabold !text-[22px]">
  ...
  return (
    <>
      ...
      <Link href={"/about"} className="grow">
        ...
        <p className="text-center -translate-y-7 font-extrabold !text-[22px]">
          معرفی اجمالی توانا
        </p>
      </Link>
...
const buyBourse = () => {
  return (
    <>
      <Link
        href={"/buy-car"}
        className="relative md:h-[50vh] grow md:basis-2/3 basis-1/3 md:w-3/5 w-full mx-auto "
      >
      ...
      <div className="md:basis-1/3 grow md:flex md:flex-col space-y-8 md:h-[50vh] md:mr-10 ">
        <Link
          href={
            "https://tavana.ebgo.ir/GetBourseCode/getInformation?src=harmony"
          }
          className="relative block grow h-[200px]"
          target={"_blank"}
          rel="noreferrer"
        >
          <div className="relative m-auto bg-[url(/photos/sign-in-for-car.png)] bg-cover bg-no-repeat bg-center md:bg-left md:mr-auto mx-auto w-[75%] md:h-[80%] h-full">
            ...
            <p className="absolute w-full text-center -bottom-5 font-extrabold !text-[22px]">
...
export default function Appbar(props: any) {
  ...
  useEffect(() => {
    if (buyCar) {
      if (scrollDir === "down") {
        ...
        window.onscroll = () => window.pageYOffset === 0 && setExpand(true);
      }
    } else {
      if (scrollDir === "up") {
        setExpand(true);
  ...
  return (
    <div className="fixed top-0 w-full text-white z-[100]">
      <div
        className={`relative flex items-center bg-black transition-all ${
          !expand ? "py-4" : "py-4"
        }`}
      >
        {expand ? (
          <button
            className="absolute lg:hidden block right-5"
            onClick={() => {
              setOpenMob(!openMob);
              if (openMob) {
                document.getElementsByTagName("html")[0].className = "";
        ...
        <Link href={"/"} className="mx-auto flex flex-col">
          ...
          <span
            className={`mt-2 md:ml-0 ml-[29px] transition-all duration-1000 ${
              !expand ? "opacity-0 h-0" : "opacity-1 h-[31px]"
            }`}
          >
            <Image
              src={"/tavana/tavana-name.svg"}
              height={100}
              width={100}
              alt={"tavana"}
              style={{ height: "auto", width: "auto" }}
      ...
      <div
        className={`relative lg:flex hidden justify-center space-x-reverse gap-10 overflow-hidden duration-200 bg-black/95 transition-all ${
          !expand ? "h-0" : "h-[62px]"
        }`}
      >
          ...
          if (menu?.link) {
            return (
              <Link
                ...
                key={menu.title}
                className={
                  "flex items-center font-semibold leading-7 text-[20px] py-[3px] px-[15px]"
                }
              >
                {menu.title}
              </Link>
            ...
            return (
              <div
                key={menu.title}
                className={
                  "flex items-center cursor-pointer font-semibold leading-7 text-[20px] py-[3px] px-[15px]"
                }
                onClick={() => openSub(menu)}
              >
                {menu.title}
                <span className="mr-2">
      ...
      <div className={`relative w-full bg-black/95 transition-all`}>
        <div
          className={`flex flex-col text-center lg:hidden space-y-12 overflow-y-auto ${
            openMob ? "h-screen pt-10 pb-52 " : "h-0"
          }`}
        >
          {component ? (
            <>
      ...
      <div
        className={`relative lg:flex hidden bg-black/95 w-full transition-all flex-col lg:flex-row justify-between duration-500 overflow-hidden px-[5vw] ${
          open ? "h-screen pt-40" : "h-0"
        }`}
      >
        {component}
        <button
      ...
      {buyCar ? (
        <div
          className={`relative justify-center lg:gap-40 gap-8 py-4 items-center flex overflow-hidden  bg-black `}
          suppressHydrationWarning
        >
          <div className="hidden lg:block font-[900] text-[16px]">
            خرید خودرو در بورس
          </div>
          <div className="flex items-center space-x-5 space-x-reverse text-[16px] font-[600]">
            <div
              className="cursor-pointer"
              onClick={() => handleClickScroll("arze")}
            >
              برنامه عرضه{" "}
            </div>
            <div
              className="cursor-pointer hidden lg:block"
              onClick={() => handleClickScroll("compare")}
            >
              مقایسه قیمت
            </div>
            <div
              className="cursor-pointer"
              onClick={() => handleClickScroll("rahnama")}
            >
              راهنما
            </div>
          </div>
          ...
          >
            <Button
              className={
                "border-white leading-8 py-0 hover:bg-white hover:text-black"
              }
            >
              دریافت کد بورس کالا
